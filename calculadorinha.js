...
obterEntrada("Insira o primeiro número: ", (entrada1) => {
    ...
    obterEntrada("Insira o segundo número: ", (entrada2) => {
        ...
        if (numero2 === null) return console.log("Entrada inválida. Por favor, reinicie e insira um número válido.");

        obterEntrada("Selecione a operação desejada:\n1 - Adição (+)\n2 - Subtração (-)\n3 - Multiplicação (*)\n4 - Divisão (/)\n5 - Porcentagem (%)\n", (entradaOperacao) => {
            const operacao = validarOperacao(entradaOperacao);
            if (operacao === null) return console.log("Opção inválida. Por favor, reinicie e selecione uma operação válida.");

            const resultado = calcular(numero1, numero2, operacao);
            console.log(`O resultado da operação é: ${resultado}. Obrigado por utilizar a calculadorinha! :D`);
            process.stdin.pause();
        });
    });
});
